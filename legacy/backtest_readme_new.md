...
### Recommended: Codex-Backed Lifecycle Backtest

For the newer buy/sell-separated engine, run `codex-backed` from the repository root:
...
If `--tickers` is omitted, the run uses the broad default universe in:

```text
codex-backed/configs/backtest_ticker_universe_config.json
```

Use `--tickers AAPL,MSFT` only for smoke tests or temporary focused runs:
...
Validate codex-backed config and tests after changing ticker universe or strategy JSON:
...
Use `--workers 1` in Codex sessions. Multiprocessing can be faster locally, but the single-worker path is the reliable default in the Codex sandbox.

### Legacy Backend Backtest

The commands below run the older `backend/backtest` pipeline. Use these when you specifically want the legacy phase-based report and fixed-horizon outcomes.
...
The ticker must also exist in the configured codex-backed price cache, currently `codex-backed/cache/prices.pkl`, or it will not contribute feature rows/trades until the cache is refreshed or expanded.

For the legacy `backend/backtest` pipeline, edit `BACKTEST_TICKERS` in `backend/backtest/config.py`. Add the ticker's sector ETF to `SECTOR_ETF_MAP`. Run with `--force-refresh` to download new data.
